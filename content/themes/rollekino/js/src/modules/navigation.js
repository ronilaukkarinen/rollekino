...
(function ($) {
  ...
  $('.menu-item a, .dropdown button').on('keyup', function (e) {
    if ($('.dropdown').find(':focus').length !== 0) {
      ...
      if (e.code === 'Escape') {
        ...
        // jscs:enable
        screenReaderSpan.text(rollekino_screenReaderText.expand);
    ...
    if (window.innerWidth > responsivenav) {
      ...
      screenReaderSpanPrev.text(rollekino_screenReaderText.expand);
      ...
      screenReaderSpanNext.text(rollekino_screenReaderText.expand);
    }
  });
  ...
  $('.dropdown-toggle').each(function () {
    $(this).attr('aria-label', `${rollekino_screenReaderText.expand_for} ${$(this).prev().text()}`);
  });
  ...
  siteHeaderMenu.find('.dropdown-toggle').on('click', function (e) {
    if (enterPressed || window.innerWidth < responsivenav) {
      ...
      $(this).attr(
        'aria-label',
        $(this).attr('aria-label') === `${rollekino_screenReaderText.collapse_for} ${$(this).prev().text()}`
          ? `${rollekino_screenReaderText.expand_for} ${$(this).prev().text()}`
          : `${rollekino_screenReaderText.collapse_for} ${$(this).prev().text()}`,
      );
    }
  });
  ...
  function mobileNav() {
    ...
    menuToggle.on('click', function () {
      ...
      $('#nav-toggle-label').text(
        ...
        $('#nav-toggle-label').text() === rollekino_screenReaderText.expand_toggle
          ? rollekino_screenReaderText.collapse_toggle
          : rollekino_screenReaderText.expand_toggle,
      );

      $(this).attr(
        'aria-label',
        $(this).attr('aria-label') === rollekino_screenReaderText.expand_toggle
          ? rollekino_screenReaderText.collapse_toggle
          : rollekino_screenReaderText.expand_toggle,
      );
